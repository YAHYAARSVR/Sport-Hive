...
class _SoccerPageState extends State<SoccerPage> with SingleTickerProviderStateMixin {
  ...
  final List<int> capacities = [10, 12, 14, 16, 15];

  final List<String> stadiumImages = [
    "images/6yard.football.jpeg",
    "images/adnan.football.jpeg",
    "images/ahmed.football.jpeg",
    "images/ayla.football.jpeg",
    "images/shafa.football.jpeg",
  ];

  final List<String> mapLocations = [
    "https://maps.app.goo.gl/rZQXZ5bdkLhRjX6f9",
    "https://maps.app.goo.gl/5H5jY3GRT6D9nKe47",
    "https://maps.app.goo.gl/mDms5pVPBp5TwYh28",
    "https://maps.app.goo.gl/fPFEqvp7TPpg52dY9",
    "https://maps.app.goo.gl/Eq8bRVbwxKAjpUsN9",
  ];
  ...
  Future<void> ensureMatchesExist() async {
    for (int dayIndex = 0; dayIndex < fullDates.length; dayIndex++) {
      for (int matchIndex = 0; matchIndex < 5; matchIndex++) {
        ...
        final stadium = "National Soccer Stadium";
        ...
        if (snapshot.docs.isEmpty) {
          await FirebaseFirestore.instance.collection('sports').add({
            ...
            'imageUrl': stadiumImages[matchIndex],
  ...
  Widget _buildMapLink(String mapLocation) {
    return GestureDetector(
      ...
      child: Row(
        children: [
          const Icon(Icons.location_pin, color: Colors.orange, size: 16),
  ...
  String _getMatchTime(int index) {
    DateTime baseDate = DateTime(2025, 4, 1).add(Duration(days: selectedDayIndex));
    DateTime baseMatchStart = DateTime(baseDate.year, baseDate.month, baseDate.day, 18, 0);
    DateTime matchStart = baseMatchStart.add(Duration(minutes: index * 105));
    DateTime matchEnd = matchStart.add(const Duration(minutes: 90));
    return "${_formatTime(matchStart)} - ${_formatTime(matchEnd)}";
  }
    ...
    return Stack(
      children: [
        ...
        Scaffold(
          ...
          appBar: AppBar(
            title: const Text('Soccer Matches', style: TextStyle(color: Colors.white)),
            centerTitle: true,
      ...
      child: Column(
        children: [
              ...
              itemBuilder: (context, index) => GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: index == selectedDayIndex ? Colors.orange : Colors.black.withValues(alpha: 0.3),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(days[index]['day']!, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Text(days[index]['date']!, style: const TextStyle(color: Colors.white70, fontSize: 12)),
                    ],
                  ),
                ),
              ),
          ...
          StreamBuilder<QuerySnapshot>(
            stream: FirebaseFirestore.instance.collection(collectionName).where('sport', isEqualTo: 'Soccer').snapshots(),
            builder: (context, snapshot) {
              if (!snapshot.hasData) {
                return const Center(child: CircularProgressIndicator());
              }

              final selectedDate = DateTime(2025, 4, 1).add(Duration(days: selectedDayIndex));
              ...
              return Column(
                ...
                children: List.generate(docs.length, (index) {
                  ...
                  return _buildMatchCard(
                    ...
                    initialData: data,
                    imagePath: stadiumImages[index % stadiumImages.length],
  ...
  Widget _buildMatchCard({
    ...
    final teamInfo = matchIndex % 2 == 0 ? "Soccer Club A" : "Soccer Club B";
    final stadium = initialData['location'] ?? 'Unknown';
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      isTeamMatch
                          ? "${(capacity / 2).round()}v${(capacity / 2).round()}"
                          : registered < capacity
                              ? "$registered/$capacity players registered"
                              : "Booking Full",
                      style: TextStyle(
