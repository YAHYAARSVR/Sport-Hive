...
class _VolleyballPageState extends State<VolleyballPage> with SingleTickerProviderStateMixin {
  ...
  final List<int> capacities = [12, 12, 12, 12, 12];

  final List<String> matchImages = [
    'images/vollyball1.jpg',
    'images/volly2.jpg',
    'images/volly3.jpg',
    'images/volly4.jpg',
    'images/volly5.jpg',
  ...
  final List<String> mapLocations = [
    "https://maps.app.goo.gl/volly1",
    "https://maps.app.goo.gl/volly2",
    "https://maps.app.goo.gl/volly3",
    "https://maps.app.goo.gl/volly4",
    "https://maps.app.goo.gl/volly5",
  ];
  ...
  Future<void> ensureMatchesExist() async {
    for (int dayIndex = 0; dayIndex < fullDates.length; dayIndex++) {
      for (int matchIndex = 0; matchIndex < 5; matchIndex++) {
        ...
        final stadium = "National Volleyball Arena";
        ...
        if (snapshot.docs.isEmpty) {
          await FirebaseFirestore.instance.collection('sports').add({
            ...
            'gender': matchIndex.isEven ? 'Male' : 'Female',
            'price': matchIndex.isEven ? '5 JD' : '7 JD',
            'ageRange': '16-60',
  ...
  Future<void> fetchTotalPlayers() async {
    final snapshot = await FirebaseFirestore.instance
        .collection('sports')
        .where('sport', isEqualTo: 'Volleyball')
        .get();
          ...
          if (date == fullDates[dayIndex]) {
            for (int matchIndex = 0; matchIndex < 5; matchIndex++) {
              if (_getMatchTime(matchIndex) == time &&
                  location == "National Volleyball Arena") {
    ...
    return Stack(
      children: [
        ...
        Scaffold(
          ...
          appBar: AppBar(
            title: const Text('Volleyball Matches', style: TextStyle(color: Colors.white)),
            centerTitle: true,
          ...
          bottomNavigationBar: BottomNavigationBar(
            ...
            items: const [
              ...
              BottomNavigationBarItem(icon: Icon(Icons.sports_volleyball), label: 'Fields'),
              ...
              final docs = snapshot.data!.docs.where((doc) {
                try {
                  ...
                  return docDate.day == selectedDate.day &&
                      ...
                      doc['location'] == "National Volleyball Arena";
                } catch (_) {
                  return false;
                }
              }).toList();
  ...
  Widget _buildMatchCard({
    ...
    final location = initialData['location'] ?? 'Unknown';
    final mapLocation = initialData['mapLocation'] ?? location;
    ...
    final capacity = int.tryParse(players[1]) ?? 12;
    final teamInfo = matchIndex.isEven ? 'Volleyball Club A' : 'Volleyball Club B';
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(location, style: const TextStyle(fontSize: 14, color: Colors.white60)),
                      const SizedBox(height: 4),
                      GestureDetector(
                        onTap: () async {
